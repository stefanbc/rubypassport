...
import { Image as ImageIcon, Download, Trash2, Printer, Loader2, RotateCcw, Pencil } from 'lucide-react';
...
export function ResultPanel({
  ...
  useEffect(() => {
    const handleBeforeUnload = (event: BeforeUnloadEvent) => {
      ...
      event.returnValue = t('components.panels.result.before_unload_prompt');
    };
  ...
  return (
    <div className={`bg-white dark:bg-zinc-900 rounded-lg p-4 sm:p-6 border border-red-200 dark:border-red-800/50 dark:ring-1 dark:ring-white/5 h-full flex flex-col transition-shadow duration-200 overflow-y-auto ${!isMobile && 'shadow-xl hover:shadow-2xl'}`}>
      <div className={`flex justify-between items-center ${capturedImage && !isProcessingImage ? 'mb-2' : 'mb-4'}`}>
        <h2 className="text-lg sm:text-xl font-semibold text-red-600 dark:text-red-400 select-none">{t('components.panels.result.title')}</h2>
          ...
          {capturedImage && !isProcessingImage && (
            <>
              <button
                ...
                title={t('components.panels.result.edit_details_tooltip')}
              >
                <Pencil size={isMobile ? 18 : 16} />
                {!isMobile && <span>{t('components.panels.result.details_button')}</span>}
              </button>
              <button onClick={onRetake} className={`flex items-center gap-1.5 text-sm text-red-500/80 hover:text-red-600 dark:text-red-300/80 dark:hover:text-red-300 transition-colors transition-transform duration-150 hover:-translate-y-0.5 rounded-md bg-red-100/50 hover:bg-red-100 dark:bg-zinc-800 dark:hover:bg-zinc-700 cursor-pointer ${isMobile ? 'p-2.5' : 'py-2 px-3'}`}>
                ...
                {!isMobile && <span>{t('components.panels.result.retake_button')}</span>}
              </button>
            </>
          )}
          {isDetailsPopoverOpen && (
            ...
            >
              <div className="space-y-2">
                <label htmlFor="personName" className="text-gray-800 dark:text-gray-100 text-sm font-medium select-none">{t('components.panels.result.person_name_label')}</label>
                <input id="personName" type="text" value={personName} onChange={(e) => setPersonName(e.target.value)} placeholder={t('components.panels.result.optional_placeholder')} className="w-full bg-gray-100 dark:bg-black text-gray-800 dark:text-white text-sm py-2 px-3 rounded border border-red-200 dark:border-red-900/40" />
              </div>
              <hr className="border-gray-200 dark:border-zinc-700" />
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <label htmlFor="watermarkEnabled" className="text-gray-800 dark:text-gray-100 text-sm font-medium select-none cursor-pointer flex-grow">{t('components.panels.result.enable_watermark_label')}</label>
                ...
                <div className="relative">
                  <input type="text" value={watermarkText} onChange={(e) => setWatermarkText(e.target.value)} placeholder={t('components.panels.result.watermark_text_placeholder')} disabled={!watermarkEnabled} className="w-full bg-gray-100 dark:bg-black text-gray-800 dark:text-white text-sm py-2 px-3 rounded border border-red-200 dark:border-red-900/40 disabled:opacity-50 disabled:cursor-not-allowed" />
                  <button onClick={() => setWatermarkText('💎 RUBY PASSPORT')} disabled={!watermarkEnabled} className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300 disabled:opacity-0" title={t('components.panels.result.reset_watermark_tooltip')}>
                    <RotateCcw size={14} />
                  </button>
                </div>
              </div>
      ...
      >
        ...
        {isProcessingImage && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/70 dark:bg-zinc-900/70 backdrop-blur-sm transition-opacity duration-300">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-300">{t('components.panels.result.processing')}</p>
          </div>
        )}
        ...
        {!capturedImage && !isProcessingImage && (
          <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-600 dark:text-gray-300">
            ...
            <p className="select-none text-sm text-gray-500 dark:text-gray-400">{t('components.panels.result.placeholder')}</p>
          </div>
        )}
