    ...
    <footer className="mt-6 pt-4 border-t border-red-200/80 dark:border-red-900/20 text-xs text-gray-500 dark:text-gray-400">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-y-2 gap-x-4">
        ...
        <div className="text-center sm:text-left">
          <p>
            <span className="hidden sm:inline">{t('components.footer.copyright', { year })}</span>
            <span className="sm:hidden">{t('components.footer.copyright_mobile', { year })}</span>
          </p>
        </div>

        {/* Right side */}
        <div className="flex flex-wrap items-center justify-center sm:justify-end gap-x-4 gap-y-2">
          <span>{t('components.footer.madeWith')} <a href="https://stefancosma.xyz" className="text-red-600/80 hover:text-red-500 dark:text-red-300/80 dark:hover:text-red-300" target="_blank" rel="noopener noreferrer">Stefan</a></span>
