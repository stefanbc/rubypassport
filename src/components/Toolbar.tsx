...
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger, DropdownMenuSub, DropdownMenuSubTrigger, DropdownMenuSubContent } from './ui/DropdownMenu';
...
export function Toolbar({ onToggleFullscreen, onManageFormatsClick, selectedFormatLabel }: HeaderProps) {
    ...
    return (
        <TooltipProvider delayDuration={200}>
            <div className={`flex items-center justify-end bg-white dark:bg-zinc-900 border border-red-200 dark:border-red-800/50 dark:ring-1 dark:ring-white/5 py-1 px-2 sm:px-4 rounded-full ${!isMobile ? 'shadow-xl hover:shadow-2xl' : ''}`}>
                {/* Desktop Toolbar */}
                <ToolbarButton onClick={onManageFormatsClick} title={t('tooltips.changeFormat')} className={`flex gap-2 text-sm text-gray-600 dark:text-gray-300 bg-gray-200 dark:bg-zinc-800 ${isMobile ? 'p-2' : 'px-3 py-1.5'}`} defaultStyle>
                    {!isMobile && <span className={isTablet ? 'truncate max-w-32 xl:max-w-48' : ''}>{selectedFormatLabel}</span>}
                    <SlidersHorizontal size={20} />
                </ToolbarButton>

                <ToolbarButton
                    onClick={handleMultiCaptureToggle}
                    title={hasQueue ? t('tooltips.viewPhotoQueue', { count: captureQueue.length }) : (multiCaptureEnabled ? t('tooltips.photoBoothEnabled') : t('tooltips.enablePhotoBooth'))}
                ...
                {!isMobile && (
                    <>
                            ...
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    <DropdownMenuTrigger asChild>
                                        <ToolbarButton title={t('tooltips.changeLanguage')} defaultStyle>
                                            <Languages size={20} />
                                        </ToolbarButton>
                                    </DropdownMenuTrigger>
                                </TooltipTrigger>
                        ...
                        <ToolbarButton onClick={onToggleFullscreen} title={isFullscreen ? t('tooltips.exitFullscreen') : t('tooltips.enterFullscreen')} defaultStyle>{isFullscreen ? <Minimize size={20} /> : <Maximize size={20} />}</ToolbarButton>
                        <ToolbarButton onClick={() => setActiveDialog('shortcuts')} title={t('tooltips.showShortcuts')} defaultStyle><Keyboard size={20} /></ToolbarButton>
                    </>
                )}
                {isMobile && (
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild><ToolbarButton title={t('tooltips.moreOptions')} defaultStyle><Menu size={20} /></ToolbarButton></DropdownMenuTrigger>
                        <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={toggleTheme} className="text-base py-2 px-3">
                                {theme === 'light' ? <Moon size={18} className="mr-3" /> : <Sun size={18} className="mr-3" />}
                                <span>{theme === 'light' ? t('tooltips.switchToDark') : t('tooltips.switchToLight')}</span>
                            </DropdownMenuItem>
                            <DropdownMenuSub>
                                ...
                                <DropdownMenuSubContent>
                                    {languages.map((language) => (
                                        <DropdownMenuItem key={language.value} onClick={() => i18n.changeLanguage(language.value)} disabled={i18n.language === language.value} className="text-base py-2 px-3">{language.label}</DropdownMenuItem>
                                    ))}
                                </DropdownMenuSubContent>
                            </DropdownMenuSub>
                            <DropdownMenuItem onClick={() => setActiveDialog('info')} className="text-base py-2 px-3"><Info size={18} className="mr-3" />{t('tooltips.showInfo')}</DropdownMenuItem>
                        </DropdownMenuContent>
                    </DropdownMenu>
                )}
            </div>
